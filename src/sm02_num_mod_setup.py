...
#MOdflow model grid setup
   ...
   # fig.savefig(f'{fig_fol}/porosity_{mod_id}.jpg',dpi=450, bbox_inches='tight')
    ...
    rch_data = {}      # Initialize an empty dictionary to store recharge arrays per stress period
    drn_data = {} 
    #
    ...
    for a in range(len(sp_sealevel)):
        ...
        sea_level_idx=init_sl_idx+abs(np.round(sea_level/dz))
        ghb_input_lst = []
        chb_input_lst = []
        ssmdata = []
        inland_head=10
        ...
        for i in range(ncol):
            #   select the active cells only
                ...
                if len(col_cells) > 0: 
                    ...
                    if ibound_sm[lay_idx, row, i] == 1.0 and botm[lay_idx] + dz > sea_level: #TODO double check this if artifacts sdtill persist
                        cond_val = vk_arr[lay_idx, row, i]*1000 #i.e (delc * delr) / dz 
                        rel_elev=dz*abs(lay_idx-sea_level_idx) #calculating the elevation of the cell relative to the new sealevel.
                        ghb_input_lst.append([lay_idx, row, i, rel_elev, cond_val])
                        ssmdata.append([lay_idx, row, i, 0.0, itype['GHB']])
                        #recharge
                        rch_arr_sp[row, i] = rch_val
                        #drainage
                        drn_input_lst.append([lay_idx, row, i, rel_elev, cond_val])  
        ...
        rch_data[a]= rch_arr_sp.copy()  #append the recharge array from the stress period to a dict
    ...
    # rch_data = {}      # Initialize an empty dictionary to store recharge arrays per stress period
    # drn_data = {}      # Dictionary for drainage data
    # row=0
    # #   only apply recharge to the cells above sea level, for each column find the first active layer

    # for sp_idx, sea_level in enumerate(sp_sealevel):

    #     sea_level_idx=init_sl_idx+abs(np.round(sea_level/dz))
    #     #initializing the recharge array of zeros
    #     rch_arr_sp = np.zeros((ibound_sm.shape[1],ibound_sm.shape[2]))
    #     drn_input_lst = []
    #     for col_idx in range(ncol):
    #             column_ibound = ibound_sm[:, row, col_idx] # Get the vertical profile for this columncolumn_ibound = ibound_sm[:, row_idx, col_idx] # Get the vertical profile for this column
    #             # Find the index of the first '1' (active cell) from the top
    #             act_lay_ind = np.where(column_ibound == 1)[0]
    #             if len(act_lay_ind) > 0:
    #                 top_act_lay_idx = act_lay_ind[0]
    #                 top_act_elev = dz*abs(top_act_lay_idx-sea_level_idx) #calculating the elevation of the cell relative to the new sealevel.
    #                 #   if the top elevation of the cell is above sea level then assign recharge to it
    #                 if top_act_elev > sea_level:
    #                     #print(top_act_elev)
    #                     rch_arr_sp[row, col_idx] = rch_val
    #                     #drainage
    #                     cond_cell = vk_arr[top_act_lay_idx, row, col_idx]*1000#(vk_arr[top_act_lay_idx, row, i] * delc * delr) / dz           
    #                     drn_input_lst.append([int(top_act_lay_idx), row, col_idx, top_act_elev, cond_cell])          
    #     # creating a list of rch arrays equal to the length of the no of stress period.
    #     rch_data[sp_idx]=rch_arr_sp.copy()  #append the recharge array from the stress period to a dict
    #     drn_data[sp_idx] = drn_input_lst
    #     #   write the final output dictionary, inlcude each stress period
    ...
    ssm = fp.mt3d.Mt3dSsm(swt, stress_period_data=ssm_arr_in)
