...
#MOdflow model grid setup
   ...
   # fig.savefig(f'{fig_fol}/porosity_{mod_id}.jpg',dpi=450, bbox_inches='tight')
    ...
    extent = [0, ibound_2d.shape[1], ibound_2d.shape[0], 0] # [xmin, xmax, ymax, ymin] for imshow
    
    # Create the plot
    fig, ax = plt.subplots()
    
    # Plot the different cell types with distinct colors
    im_active = ax.imshow(masked_active, cmap=plt.cm.viridis, vmin=0.5, vmax=1.5, extent=extent, label='Active (1)')
    im_inactive = ax.imshow(masked_inactive, cmap=plt.cm.gray, vmin=-0.5, vmax=0.5, extent=extent, label='Inactive (0)')
    im_negative = ax.imshow(masked_negative, cmap=plt.cm.autumn, vmin=-1.5, vmax=-0.5, extent=extent, label='Negative (-)')
    
    # Add labels and title
    ax.set_xlabel('columns')
    ax.set_ylabel('layers')
    ax.set_title('iBound Array Visualization (z vs x)')
    ...
    # Add the legend
    ax.legend(handles=[active_patch, inactive_patch, negative_patch])
    
    
    # Add the legend
    ax.legend(handles=[active_patch, inactive_patch, negative_patch], bbox_to_anchor=(1.05, 1), loc='upper left')
