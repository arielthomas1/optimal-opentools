...
for i in range(1,5):#num_models+1):
    ...
    #INterporlateing for higher resolution  time series 
    orig_yr=1000*sl_arr[:,0]
    orig_sl=sl_arr[:,2]
    f_interpolate = interp1d(orig_yr, orig_sl, kind='cubic')
    
    yr_new = np.arange(orig_yr.min(), orig_yr.max()+1, 500)
    
    # 5. APPLY THE FUNCTION TO GET THE NEW SEA LEVELS
    sl_new = f_interpolate(yr_new)
    
    # 6. COMBINE INTO A NEW ARRAY
    sp_sealevels_500yr = np.column_stack((yr_new, sl_new))
    
    # --- Optional: Print and Plot to Verify ---
    
    # Print the first 10 rows of the new data
    print("Original Data Shape:", sl_arr.shape)
    print("Resampled Data Shape:", sp_sealevels_500yr.shape)
    print("\nFirst 10 rows of resampled data:")
    print(sp_sealevels_500yr[:10])
    
    # Plotting the results
    plt.figure(figsize=(10, 6))
    plt.plot(orig_yr, orig_sl, 'o', markersize=8, label='Original Data (2000 yr)')
    plt.plot(yr_new, sl_new, 'x',markersize=8, label='Resampled Data (500 yr)')
    plt.xlabel("Years Before Present")
    plt.ylabel("Sea Level (meters)")
    plt.title("Sea Level History - Resampled Data")
    plt.legend()
    plt.grid(True)
    plt.show()
...
#MOdflow model grid setup
   ...
   # fig.savefig(f'{fig_fol}/porosity_{mod_id}.jpg',dpi=450, bbox_inches='tight')
    ...
    #         -31.8,  -10.4,    0. ]
    #RSL at 500 yr intervals
    sp_sealevel=sp_sealevels_500yr[:,1]
    ...
    nstp = 10*np.ones_like(sp_sealevel) # number of time steps per period ca. 500 years per time step (???)
